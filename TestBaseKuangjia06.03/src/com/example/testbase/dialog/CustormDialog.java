...
public class CustormDialog extends Dialog implements DialogInterface {
	 ...
	 *            对话框内容
	 * @param theme
	 *            对应的style 这里为R.style.CustomDialog_1 可自定义style
	 * @param dialogcallback
	 *            确定取消按钮的回调 分别是 onCancle onOk
	 * @param index
	 *            显示几个button 1 为只有一个确定键，其他为有确定取消两个按钮
	 * 
	 *            调用实例 dialog = new CustormDialog(SettingsActivity.this,"缓存清理",
	 *            "点击确定为您清理以下历史信息:\n系统通知，提箱小票，行业资讯，装箱单录入",
	 *            R.style.CustomDialog_1, new DialogCallBack(){
	 * @Override public void OkDown() { dialog.dismiss(); //这里放 确定按钮响应 }
	 * @Override public void CancleDown() { dialog.dismiss(); //这里放取消按钮响应 }
	 *           },2);
	 */
